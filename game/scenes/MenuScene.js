...
export default class MenuScene extends Phaser.Scene {
    ...
    showMainMenu() {
        this.switchTo((c) => {
            ...
            c.add(this.add.text(80, 210, `💰 ${PlayerData.getMoney()} coins`,    { fontSize: '18px', color: '#ffdd00' }));
            c.add(this.add.text(80, 234, `🏆 Best: ${PlayerData.getHighscore()}`, { fontSize: '16px', color: '#aaffaa' }));
    ...
    showEndlessMenu() {
        this.switchTo((c) => {
            ...
            this.addButton(c, 80, 260, "Solo",  () => this.showDifficultyPicker(false));

            // Co-op → same difficulty picker but passes coop = true
            this.addButton(c, 80, 340, "Co-op", () => this.showDifficultyPicker(true));
    ...
    addButton(container, x, y, label, callback) {
        const btn = this.add.text(x, y, label, {
            fontSize: '28px', color: '#ffffff', backgroundColor: '#000000', padding: { x: 10, y: 5 },
        }).setInteractive();
        btn.on('pointerover',  () => btn.setStyle({ color: '#ffff00' }));
    ...
    addToggleButton(container, x, y, label, key) {
        ...
        const getStyle = () => ({
            color: this.optionsState[key] ? '#0f0' : '#f00'
        });
